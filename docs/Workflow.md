...
`backlog` is not selected by `next`; move a ticket to a ready status when it should enter the automation queue. `promote <ticket-id> [--to <status>]` is the sanctioned way to do this: without `--to` it derives the type-appropriate entry status from `routing.doneRequires`/`workflow.pipelineOrder` (e.g. `ready_for_decomposition` for epics/stories, `ready_for_design` for tasks/bugs on the scaffold); `--to <status>` overrides it with any other trigger status. It refuses tickets not currently in `backlog`, warns (non-fatally) on open `blockedBy` dependencies, and records the promotion in the Run Log.

For agent-run flows, backlog -> `ready_*` is a user approval boundary: agents may report backlog diagnostics, but they use `promote` only for tickets covered by an explicit instruction in the current session.
...
`git.autoVersionBump` (default `false`) gates an automatic semver bump inside `fast-forward` itself: when `true`, every `fast-forward` call checks whether installable payload changed (skills, agents, prompts, hooks, `src/`, `bin/`, `README.md`, `SKILL.md` — the same set `local-board install` copies and `install --status` hashes; `install.mjs` is excluded) since the last processed point, and if so computes a semver level from the merged ticket types in that range (`bug` → patch, `task`/`story` → minor, `epic` → major; max over a multi-merge range; unparseable/unresolved merges fall back to a flat patch), rewrites `package.json`, and commits it as `chore: bump version <X.Y.Z>`. Idempotence is a single-owner, compare-and-swap-written marker ref (`refs/local-board/version-bump-head`) pointing at the resulting bump commit; "already processed" is tested by ancestry, not by commit subject, so a same-clone re-run and a fresh clone both correctly no-op.

The bump range is NOT `fast-forward`'s own before/after pair (its `previousHead`/`newHead`, reflected in the `advanced` field). In this repo's real closeout the merge lands directly in the project root checkout (`git merge --no-ff` on the default branch, per every closeout skill contract) before `fast-forward` ever runs, so `previousHead === newHead` (`advanced: false`) even when a real bump is due — nothing *external* moved this checkout out from under it. `fast-forward` instead lets the bump self-resolve its own range: base = marker → last `chore: bump version` commit → repo root commit, tip = the checkout's current `HEAD`. That range is a superset of `fast-forward`'s own before/after pair, so it correctly covers both an in-checkout merge and the external-advance case (a linked worktree pushing a ref this checkout then adopts via `fast-forward`).
...
Manual escape hatch: `local-board version-bump [--level major|minor|patch] [--range <a>..<b>] [--json]` shares the same underlying logic and, with no arguments, resolves the identical marker-based range. `--level` forces the level and skips the merge-subject scan/ticket lookup. `--range <a>..<b>` is an explicit override for one-off use. Recovery from a `fast-forward`-triggered bump failure is a plain no-range `local-board version-bump` re-run: a commit failure restores `package.json` to its pre-write bytes and leaves the marker unmoved, so the re-run's self-resolved range recomputes the exact same target rather than double-incrementing (no `--range` argument is needed). A dirty `package.json` at entry is refused (`dirty-package-json`, exit `2`, no write). Exit codes: `0` on success (a bump or an intentional no-op), `2` on a precondition refusal (`not-enabled`, `not-on-default`, `dirty-package-json`).
