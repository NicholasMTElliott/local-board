...
`gate-check` and `specialty-run` do not invoke agents. Ticket evidence (`completedSteps` front matter) is mutated only by `gate-check`'s empty-catalog self-certification path; non-empty consultations are recorded as ticket evidence by `gate-complete`, and specialty evidence is recorded by `complete-step`. Separately, on the non-empty-catalog branch `gate-check` (and, for a claude-subagent specialty route, `specialty-run`) stamps a short-lived consultation entry in the internal active-steps dispatch ledger — not ticket evidence — so a routing-validator hook (`local-board check-dispatch`) recognizes the upcoming gate/specialty agent dispatch as authorized. `gate-complete` clears that ledger entry after recording the consultation.
