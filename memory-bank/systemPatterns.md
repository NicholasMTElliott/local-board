...
The `## CLI Commands` blocks in root `SKILL.md` and `skills/codex/local-board/SKILL.md` are curated to 35 commands, byte-identical, and enforced by `test/skill-usage-sync.test.js`; root `SKILL.md` is canonical.
The four skill flows (single-ticket/team for Claude/Codex) document the design-review step; `skill-usage-sync` enforces a `REQUIRED_COMMANDS` surface containing `design-review-check`, `design-review-complete`, and `promote`.
The four skill flows also encode the empty-ready backlog approval-boundary policy; `skill-usage-sync` guards the state-report/unblocked-backlog diagnostic, hard stop, and explicit-session `promote` path.
...
Dispatch verification uses `.local-board/active-steps.json` as the deterministic in-flight ledger, anchored at the main checkout's git common dir so linked worktrees share one record. `begin-step` stamps the ticket's resolved action/route/model there; `design-review-check` stamps design-review for `claude-subagent:` routes only (codex-task routes stamp nothing); gate-check (non-empty catalog) and `specialty-run` stamp scoped consultation entries (`kind: gate|specialty` with stage/action/route/model identity) using no-clobber semantics. Stamps include fallback models only when configured, and dispatch checks accept the pin or a listed fallback. Configured effort is deliberately excluded because it is a dispatch hint, not routed-work identity. `complete-step`/`approve-inline` clear the matching action identity, and specialty completion clears its matching specialty identity; gate completion clears only the matching gate-kind stage. A real status change sweeps abandoned consultation stamps, while same-status re-saves do not. `check-dispatch --agent [--model] [--ticket]` reads the ledger for hook use; with no ledger entry it resolves the ticket's registered, existence-verified worktree before falling back to the invocation root. It always emits JSON on stdout and exits 0 allow / 1 deny / 2 error while passing through non-local-board agents and unverifiable models. Claude Code enforcement hooks are opt-in via `local-board install --hooks`: routing-validator, dispatch-ledger, evidence-gate, and approve-inline-consent. Hooks fail open on errors/timeouts; CLI strict routing remains the backstop.

Ledger reads/clears (`src/active-steps.js`) split strict vs self-heal (T20260710T2051Z). Reads: strict (`readLedgerStrict`/public `readActiveSteps`) returns `{}` on ENOENT but throws on corrupt JSON/non-object-top-level/other IO errors; self-heal (`readLedgerSelfHeal`, used by all stamp/clear call sites today) swallows any read failure to `{}`. Clears: best-effort self-healing (`clearActiveStep`, `clearActiveStepIf`) vs fail-closed `clearActiveStepStrict(root, ticketId, predicate, options)` — same locked read-modify-write (factored into a shared `clearActiveStepWith` helper), but a missing entry OR a missing ledger file is still no-op success (`false`, evaluated post-lock-acquisition), while corrupt/unreadable throws with the ledger bytes untouched. `clearActiveStepStrict` is unwired (no caller): it's a necessary-but-not-sufficient precondition for a future fail-closed authoring-correction lane, not a general-purpose replacement for the self-healing clears. Lock decision: `beginStep` stays lock-free, not sharing `moveTicket`'s ticket lock — a correction lane needs that serialization (or a composable clear-under-held-lock primitive) to make clear+publish atomic against a racing `beginStep`, so the lane remains blocked on one of those two, not just on the strict clear; if either is ever adopted, ticket-lock-before-ledger-lock is the required order (matching `moveTicket`'s existing nesting), never the reverse.
...
`create` warns on stderr for type-vs-status mismatches derived from `workflow.statusActions` vs `routing.doneRequires`; the evidence-free authoring-correction lane was considered and rejected, so correction stays `move --override --reason`.
...
When `git.commitPlanningOnTransition` is true (scaffold default for new boards; false for older configs that omit the key), mutating per-ticket CLI commands commit only `plans/**` right after success via pathspec-limited staging and `git commit -- plans`, so already-staged non-planning files are never swept into transition commits. Clean planning trees and non-git roots are no-ops; dirty-check/stage/diff/commit failures degrade to stderr warnings and never roll back the mutation. `worktree-add` commits only its existing-worktree branch-field repair path, at the linked worktree root; new worktree branch stamps use their own commit. Skipped on `move done` when `git.autoMerge` also runs (that path's own planning commit is the sole committer).
`worktree-add` refuses uncommitted ticket files (untracked or dirty) with a fix-naming message; authoring flows end committed, either automatically via `git.commitPlanningOnTransition` or manually on flag-off boards.
...
## MVP CLI
Use `node ./bin/local-board.js version`, `where [--json]`, `validate`, `list`, `query-next`, `query-ticket`, `state-report`, `schema`, `create`, `promote`, `estimate`, `calibration suggest`, `gate-check`, `gate-complete`, `specialty-run`, `design-review-check`, `design-review-complete`, `start-work`, `begin-step`, `check-dispatch`, `complete-step`, `approve-inline`, `move`, `set`, `section`, `comment`, `link-parent`, `link-child`, `block`, `unblock`, `team-config`, and `init`.

`design-review-check` resolves the design-review route/model/effort, prompt path, and ticket context, and stamps active-step verification for `claude-subagent:` routes; `design-review-complete` records `design-review:<executor>@<model>` evidence through the core recorder. Both commands are gated by `routing.requireDesignReview`.

`move` changes status and relocates the ticket; when `routing.requireGateConsultation` is true it also refuses the three forward gated transitions above without a recorded consultation, when `routing.invalidateOnLoopBack` is true a move to a `ready_*` status strips at-or-downstream evidence, and when `routing.enforceTransitions` is true it refuses a target status outside `workflow.transitions` + the structural allow-set unless `--override --reason <text>` is given (see Config above). `set` updates mutable front matter fields (routes through `move` for `status`, so it is gated/invalidated/enforced too, and accepts the same `--override`/`--reason` flags for parity). `section` replaces section content; section boundaries are fence-aware for backtick/tilde fenced blocks. `comment` appends timestamped notes; repeatable `--marker key=value` uses strict charsets, renders as `[k:v ...]` after the timestamp, is annotation not evidence, and is grammar-validated fence-aware; read-only `comments` lists/filters by section and marker, with `--json`.
`promote` is the sanctioned backlog -> `ready_*` entry command: it derives the type-appropriate trigger status from config unless `--to` supplies a trigger override, refuses non-backlog sources, warns on open ticket dependencies, and writes an audited Run Log line through `moveTicket`.
...
`section --file <path>` is preferred for generated or multi-line Markdown; inline section text is for short edits. Create the `--file` target with the Write tool, never with shell redirection. The payload is section body only; omit the target `## Heading` and fence literal top-level `## ` sample lines.
`list --json`, `list --ready --json`, `query-next --json`, and `query-ticket <id> --json` records include dependency state: `parent`, `children`, `blocks`, `blockedBy`, and `blockedByOpen` (missing or non-closed blockers). `list --unblocked` keeps records with empty `blockedByOpen`, composes with `--status <status>`, and is rejected with `--ready` (exit 2).
