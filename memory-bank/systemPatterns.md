...
`design-review-check` resolves the design-review route/model/effort, prompt path, and ticket context; `design-review-complete` records `design-review:<executor>@<model>` evidence through the core recorder. Both commands are gated by `routing.requireDesignReview`.
